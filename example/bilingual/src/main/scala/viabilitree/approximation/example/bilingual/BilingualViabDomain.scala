...
object BilingualViabDomain extends App {
  ...
  val vk=KernelComputation(
    ...
    k = Some(p => p(0) <= 1 && p(0) >= 0.2 && p(1) <= 1 && p(1) >= 0.2 && (p(0)+p(1) <=1 ) && p(2)<=1 && p(2)>=0),
  ...
  saveVTK3D(viabilityDomain, s"${output}Bilingual${vk.depth}viabdil${vk.dilation}withk.vtk")
  saveHyperRectangles(vk)(viabilityDomain, s"${output}Bilingual${vk.depth}dil${vk.dilation}withk.txt")
