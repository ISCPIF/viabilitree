...
//import fr.iscpif.kdtree.structure.NonEmptyTree$
...
//    override def apply(x: Vector[Double]): Control = c
//  }
//
// def apply[CONTENT <: ControlledDynamicContent.Content] (viab: NonEmptyTree[CONTENT], controls: Vector[Control] ) = new Strategy {
//   override def apply(x: Vector[Double]): Control = viableStrategy(x,viab, controls)
// }
//}
//
...
//  def viableStrategy[CONTENT <: ControlledDynamicContent.Content] (p: Vector[Double], v: NonEmptyTree[CONTENT], controls: Vector[Control]): Control = {
